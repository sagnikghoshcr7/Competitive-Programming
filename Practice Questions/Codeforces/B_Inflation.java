...
public class B_Inflation {
    ...
    // Direct Solution
    private static void sagnik1() throws IOException {
        ...
        for (int i = 1; i < n; i++) {
            double ratio = (a[i]) / ((double) sum + toIncrease) * 100;
            if (ratio > k) {
                toIncrease += Math.ceil(((a[i] * 100) - (k * (sum + toIncrease))) / (double) (k));
            }
            sum += a[i];
        }
        out.println(toIncrease);
    }
    ...
    private static void sagnik() throws IOException {
        ...
        long[] prefix = genPrefixArr(a);

        long max = 0;
        for (int i=1; i<n; i++) {
            max = Math.max(max, solve(prefix, k, a[i], i));
        }
        out.println(max);
    }

    private static long solve(long[] prefix, int k, long element, int i) {
        long low = 0, high = 1000000000000000L, ans = 0;
        while(low<=high){
            long mid = (low+high)/2;
            if (element*100 <= k*(mid + prefix[i-1])) { ans = mid; high = mid-1; }
            else low = mid+1; 
        }
        return ans;
    }
    ...
    private static long[] genPrefixArr(long[] a) {int n = a.length; long[] pf = new long[n]; pf[0] = a[0]; for (int i=1; i<n; i++) pf[i] = pf[i-1] + a[i]; return pf; }
