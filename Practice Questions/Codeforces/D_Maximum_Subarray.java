...
public class D_Maximum_Subarray {
    ...
    private static void sagnik() throws IOException {
        ...
        for (long[] row : dp) {
            for (long ele : row) {
                answer = Long.max(answer, ele);
            }
        }
        out.println(answer);
    }

    public static Long solve(int index, int count) {
        if (index == n) {
            if (count == 0) return 0L;
            else return min;
        }
 
        if (n - index < count) return min;
 
        if (dp[index][count] != min) return dp[index][count];
 
        if (count > 0) dp[index][count] = Long.max(0L, Long.max(arr[index] + x + solve(index + 1, count - 1), arr[index] - x + solve(index + 1, count)));
        else dp[index][count] = Long.max(0, arr[index] - x + solve(index + 1, count));
