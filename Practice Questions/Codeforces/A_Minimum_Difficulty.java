...
public class A_Minimum_Difficulty {
    ...
    private static void sagnik() throws IOException {
        ...
        for(int i=0; i<n; i++){
            arr[i] = fs.nextInt();
            if (i != 0) dif = Math.max(arr[i] - arr[i - 1], dif);
        }

        int ans=-1;
        int afterDif=arr[2]-arr[0];
        for(int i=1; i<n-1; i++){
            int thisDif = arr[i + 1] - arr[i - 1];
            if (thisDif < dif) ans = dif;
            else afterDif = Math.min(afterDif, thisDif);
        }

        if (ans == -1) ans = afterDif;
        out.print(ans);
        out.flush();
    }
