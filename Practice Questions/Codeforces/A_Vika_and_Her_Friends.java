...
public class A_Vika_and_Her_Friends {
    ...
    private static void sagnik() throws IOException {
        ...
        for (int i = 0; i < k; i++) {
            ...
            // if Vika and friend are in the same type of room
            if ((dx + dy) % 2 == 0) canEscape = false;
        }
        out.println(canEscape ? "YES" : "NO");
    }
    ...
    public static class FastScanner {
        ...
        private String next() throws IOException {while(!st.hasMoreTokens()) try {st=new StringTokenizer(br.readLine());} catch (IOException e) {e.printStackTrace();} return st.nextToken();}

        private int[] setArray(int n) throws IOException {int[] a = new int[n]; for (int i=0; i<n; i++) a[i] = nextInt(); return a;}
        private long[] lsetArray(int n) throws IOException {long a[] = new long[n]; for(int i=0; i<n; i++) a[i] = nextLong(); return a;}
        private int[][] set2dArray(int n, int m) throws IOException {int[][] a = new int[n][m]; for (int i=0; i<n; i++) for (int j=0; j<m; j++) a[i][j] = fs.nextInt(); return a;}
        private long[][] lset2dArray(int n, int m) throws IOException {long[][] a = new long[n][m]; for (int i=0; i<n; i++) for (int j=0; j<m; j++) a[i][j] = fs.nextLong(); return a;}

        private int nextInt() throws IOException {return Integer.parseInt(next());}
        private Long nextLong() throws IOException {return Long.parseLong(next());}
        private double nextDouble() throws IOException {return Double.parseDouble(next());}
        private char nextChar() throws IOException {return next().toCharArray()[0];}
        private String nextString() throws IOException {return next();}
        private String nextLine() throws IOException {return br.readLine();}
        private String nextToken() throws IOException {while (st == null || !st.hasMoreElements()) {try {st = new StringTokenizer(br.readLine());} catch (IOException e) {e.printStackTrace();}} return st.nextToken();}
        private BigInteger nextBigInteger() throws IOException {return new BigInteger(next());}
    }
}