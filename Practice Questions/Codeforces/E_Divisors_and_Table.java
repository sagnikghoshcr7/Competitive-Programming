...
public class E_Divisors_and_Table {
    ...
    private static Map<Long, Long> fact(long n, Map<Long, Long> m) {
        for (long d = 2; d*d <= n; d++) {
            ...
            if (q > 0) m.put(d, m.getOrDefault(d, 0L)+q);
        }
        if (n > 1) m.put(n, m.getOrDefault(n, 0L) + 1);
        return m;
    }
    ...
    private static void sagnik() throws IOException {
        ...
        long m1 = Long.parseLong(line[1]);
        long m2 = Long.parseLong(line[2]);
        List<Long> divs = new ArrayList<Long>();
        Map<Long, Long> facts = fact(m2, fact(m1, new HashMap<>()));
        divs.add(1L);
        for (Map.Entry<Long, Long> e : facts.entrySet()) {
            long p = e.getKey();
            long q = e.getValue();
            long mult = 1;
            int size = divs.size();
        ...
        for(int i = 0; i < divs.size(); i++){
            ...
            if(div > n*n) break;
            long min_ = (div+n-1)/n;
            while (left < divs.size() && divs.get(left) < min_) {
                left += 1;
            }
            if (left == divs.size() || divs.get(left) > n) break;
            for (int right = left; right <= i; right++) {
                long row = divs.get(right);
                if (row > n) break;
                if(div % row == 0 && div / row <= n){
                    nout += 1;
                    xorout ^= row;
                    break;
                }
