...
public class E_Rearrange_Brackets {
    ...
    private static void sagnik() throws IOException {
        int k = Integer.parseInt(fs.nextLine());
        String rbs = fs.nextLine();
        String cur = rbs;
        for (int i = 0; i <= k; i++) {
    ...
    private static String remove_largest(String rbs) {
        cost = 0;
        int max = 0;
        int l = -1;
        int r = -1;
        ...
        for (int i = 0; i < rbs.length(); i++) {
            if (i != l && i != r) sb.append(rbs.charAt(i));
        }
        return sb.toString();
    }
