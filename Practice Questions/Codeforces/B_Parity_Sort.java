...
public class B_Parity_Sort {
    ...
    private static void sagnik() throws IOException {
        int n = fs.nextInt();
        long[] v = fs.lsetArray(n);
        long[] odd = new long[n], even = new long[n], res = new long[n];
        int oInd = 0, eInd = 0;

        for (int i = 0; i < n; i++) {
            if ((v[i] & 1) == 1) odd[oInd++] = v[i];
            else even[eInd++] = v[i];
        }

        Arrays.sort(odd, 0, oInd);
        Arrays.sort(even, 0, eInd);

        int oddP = 0, evenP = 0;
        for (int i = 0; i < n; i++) {
            if ((v[i] & 1) == 1) res[i] = odd[oddP++];
            else res[i] = even[evenP++];
        }

        out.println(isSorted(res) ? "YES" : "NO");
    }
