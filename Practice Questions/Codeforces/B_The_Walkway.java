...
public class B_The_Walkway {
    ...
    private static void sagnik() throws IOException {
        long n = fs.nextLong(), m = fs.nextLong(), d = fs.nextLong();
        ArrayList<Long> list = new ArrayList<>();
        for (int i = 0; i < m; i++) list.add(fs.nextLong());

        long left = 0;
        if (list.get(0) != 1) { left = 1; list.add(0, 1L); }

        list.add(n + 1);
        int len = list.size();
        long res = 0;
        for (int i=1; i<len; i++) res += helper(list.get(i - 1), list.get(i), d);
        long flag = res;
        long temp = res;
        for (int i = 1; i < len - 1; i++) {
            long curr = helper(list.get(i - 1), list.get(i + 1), d) - helper(list.get(i - 1), list.get(i), d) - helper(list.get(i), list.get(i + 1), d);
            temp = flag + curr;
        ...
        if (res == flag && left == 0) cnt += 1;
        out.println(res + " " + cnt);
    }

    private static long helper(long x, long y, long d) {
        boolean e = ((y - x) % d) == 0;
