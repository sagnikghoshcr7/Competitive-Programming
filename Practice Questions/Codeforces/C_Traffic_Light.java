...
public class C_Traffic_Light {
    ...
    private static void sagnik() throws IOException {
        int n = fs.nextInt(); char ch = fs.nextChar();
        String s = fs.next();
        if (ch == 'g') {out.println(0); return;}
        s = s+s;

        int lastG = -1, ans = Integer.MIN_VALUE;
        for (int i=(2*n)-1; i>=0; i--) if (s.charAt(i) == 'g') lastG = i;

        for (int i=(2*n)-1; i>=0; i--) {
            if (i <= n && s.charAt(i) == ch) ans = Math.max(ans, lastG - i );
            if (s.charAt(i) == 'g') lastG = i;
        }
