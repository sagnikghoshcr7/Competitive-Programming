...
public class B_Swap_and_Reverse {
    ...
    private static void sagnik() throws IOException {
        ...
        if (k % 2 == 0) {
            ...
            Arrays.sort(chArr);
            out.println(new String(chArr));
        } else {
            StringBuilder odd = new StringBuilder();
            StringBuilder even = new StringBuilder();
            for (int i = 0; i < n; i++) {
                if ((i&1) == 1) odd.append(s.charAt(i));
                else even.append(s.charAt(i));
            }

            char[] evenArr = even.toString().toCharArray(); char[] oddArr = odd.toString().toCharArray();
            ...
            while (i < evenArr.length || j < oddArr.length) {
                if (i < evenArr.length) res.append(evenArr[i++]);
                if (j < oddArr.length) res.append(oddArr[j++]);
            }
