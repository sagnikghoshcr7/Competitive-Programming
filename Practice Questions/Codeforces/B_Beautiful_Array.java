...
public class B_Beautiful_Array {
    ...
    private static void sagnik() throws IOException {
        int n = fs.nextInt(), k = fs.nextInt(), beauty = fs.nextInt(); long sum = fs.nextInt();
        if (sum/k < beauty) {out.println(-1); return;}
        
        out.print((k*beauty - 1) + " ");
        sum -= k*beauty - 1; n--;
        for (int i=1; i<=(sum); i++) //need to complete
        while (sum > 0 || n-->0) {
            if (sum > 0) {
                out.print((beauty - 1) + " ");
                sum -= beauty-1;
