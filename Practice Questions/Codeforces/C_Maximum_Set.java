...
public class C_Maximum_Set {
    ...
    private static void sagnik() throws IOException {
        ...
        int max_size = 1;
        while((l << max_size) <= r) max_size++;
        int ans = (r / (1 << (max_size - 1)) - l + 1);
        if(max_size > 1) ans += (max_size - 1) * Math.max(0, (r / (1 << (max_size - 2)) / 3 - l + 1));
        out.println(max_size + " " + ans);
        out.flush();
    }
