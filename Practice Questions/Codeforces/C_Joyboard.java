...
public class C_Joyboard {
    ...
    private static void sagnik() throws IOException {
        ...
        int y = k, one = 1, three = 3;
        
        if (y == one) out.println("1");
        else if (y > three) out.println("0");
        else {
            if (k != three - one) out.println(m - Math.max((m - n) / n, 0) - Math.min(m, n));
            else out.println(Math.max((m - n) / n, 0) + Math.min(m, n));
        }
    }
