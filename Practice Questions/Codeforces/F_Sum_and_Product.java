...
public class F_Sum_and_Product {
    ...
    private static void sagnik() throws IOException {
        ...
        for (Long i : a) map.put(i, map.getOrDefault(i, (long) 0) + 1);
                ...
                if (((int) Math.abs(x)) % 2 == (sqrt(x*x - 4*y)) % 2) {
                    ...
                    if (map.containsKey(p)) {
                        if (map.containsKey(x - p)) {
                            if (p * (x - p) == y && x - p != p) ans[in] += map.get(p) * map.get(x - p);
                            else if (x - p == p) ans[in] += (map.get(p) * (map.get(p) - 1)) / 2;
                        }
                    }

                    if (map.containsKey(ne) && ne != x - p) {
                        if (map.containsKey(x - ne)) {
                            if (ne * (x - ne) == y && x - ne != ne) ans[in] += map.get(ne) * map.get(x - ne);
                            else if (x - ne == ne) ans[in] += (map.get(ne) * (map.get(ne) - 1)) / 2;
                        }
                    }
                    in += 1;
                } else {
        ...
        for (int i = 0; i < ans.length; i++) out.print(ans[i] + " ");
        out.println();
    }
