...
public class H_The_Third_Letter {
    ...
    private static void sagnik() throws IOException {
        ...
        for (int i = 0; i < m; i++) {
            ...
            long b = fs.nextLong();
            long x = fs.nextLong();
            adj[(int) b].add(new Pair(a, -1 * x));
            adj[(int) a].add(new Pair(b, x));
        }
        ...
        for (int i = 0; i < n + 1; i++) {
            if (vis[i]) continue;
            q.add((long) i); mp.put((long) i, 0L);
            while (!q.isEmpty()) {
                long node = q.poll();
                vis[(int) node] = true;

                for (Pair it : adj[(int) node]) {
                    long x = it.first;
                    long val = it.second;
                    if (vis[(int) x]) {
                        if (mp.get(node) + val != mp.get(x)) { ok = true; break; }
                    } else {
                        mp.put(x, mp.get(node) + val);
                        vis[(int) x] = true;
                        q.add(x);
                    }
                }
