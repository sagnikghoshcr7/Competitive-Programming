...
public class E_Binary_Inversions {
    ...
    private static void sagnik() throws IOException {
        ...
        for(int i=1; i<n; i++) prefix[i] = prefix[i-1] + (a[i-1] = fs.nextInt());

        long max,ans=0;
        max = (a[n-1] = fs.nextInt()) == 1 ? prefix[n-1] : -1;
        for(int i=n-2; i>-1; i--){
            suffix[i] = suffix[i+1] + (a[i+1] == 1 ? 0 : 1);
            if(a[i] == 1) {
                max = Math.max(prefix[i]-suffix[i], max);
                ans += suffix[i];
            }
            else max = Math.max(suffix[i]-prefix[i], max);
        }
        out.println(Math.max(max, 0) + ans);
    }
