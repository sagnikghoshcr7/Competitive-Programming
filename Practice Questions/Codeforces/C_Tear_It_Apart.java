...
public class C_Tear_It_Apart {
    ...
    private static void sagnik() throws IOException {
        ...
        int ans = IntStream.rangeClosed('a', 'z')
    ...
    private static int computeOperationNum(String s, char target) {
        int result = 0, count = 0, n = s.length();
        for (int i=0; i<=n; i++) {
            if (i != n && s.charAt(i) != target) count++;
            else {
