...
public class B_Negative_Prefixes {
    ...
    private static void sagnik() throws IOException {
        int n = fs.nextInt();
        int[] a = fs.setArray(n);
        int[] lock = fs.setArray(n);

        ArrayList<Integer> unlockedVals = new ArrayList<>();
        for (int i=0; i<n; i++) if (lock[i] == 0) unlockedVals.add(a[i]);
        Collections.sort(unlockedVals);

        int ind = unlockedVals.size()-1;
        for (int i=0; i<n; i++) {
            if (lock[i] == 0) {
                a[i] = unlockedVals.get(ind);
                ind--;
            }
        }
