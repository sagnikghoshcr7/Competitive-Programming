...
public class C_Slay_the_Dragon {
    ...
    private static void sagnik() throws IOException {
        ...
        int t = fs.nextInt();
        while(t-->0) {
            long def = fs.nextLong();
            long att = fs.nextLong();
            int low = 0, high = n;
            while (low < high) {
                int mid = (low + high)/2;
                if (a[mid] < def) {
            ...
            if (low != n) {
                cost1 = Math.max(0, att - sum + a[low]);
            }
            long cost2 = Long.MAX_VALUE;
            if (low != 0) {
                long val = a[low-1];
                cost2 = def - val + Math.max(0, att - sum + val);
            }
            long cost = Math.min(cost1, cost2);
            out.println(cost);
        }
    }
