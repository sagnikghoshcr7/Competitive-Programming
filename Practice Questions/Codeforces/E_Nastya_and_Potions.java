...
public class E_Nastya_and_Potions {
    ...
    public static void rec(int i, List<List<Integer>> arr, List<Integer> c, List<Long> dp, int n) {
        if (dp.get(i) != INF) return;
        long ans = c.get(i), sum = INF;
        for (int val : arr.get(i)) {
            rec(val, arr, c, dp, n);
            if (sum == INF) sum = dp.get(val);
            else sum += dp.get(val);
        }
        dp.set(i, Math.min(ans, sum));
        return;
    }
    ...
    private static void sagnik() throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < m; j++) temp.add(fs.nextInt() - 1);
            mat.add(temp);
        }

        List<Long> dp = new ArrayList<>();
        for (int i = 0; i < n; i++) dp.add(INF);
        for (int i = 0; i < n; i++) rec(i, mat, list, dp, n);
        for (int i = 0; i < dp.size(); i++) out.print(dp.get(i) + " ");
        out.println();
    }
