...
public class B_Young_Photographer {
    ...
    private static void sagnik() throws IOException {
        ...
        if (max < min) out.println(-1);
        else if (x0 >= min && x0 <= max) out.println(0);
        else out.println(Math.abs(min - x0) < Math.abs(max - x0) ? Math.abs(min - x0) : Math.abs(max - x0));
