...
public class F_Monocarp_and_a_Strategic_Game {
    ...
    static class Vector {
        ...
        public Vector(int x, int y) {
            this.x = x;
            this.y = y;
            this.ratio = x == 0 ? -1e15 : (double) y / x;
            this.quad = quadrant(x, y);
        }

        static double getAngle(int x, int y) {
            double z = Math.atan2(y, x);
            return y >= 0 ? z : Math.PI * 2 + z;
        }
    ...
    private static void sagnik() throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            groups[i][0] = fs.nextInt();
            groups[i][1] = fs.nextInt();
            groups[i][2] = fs.nextInt();
            groups[i][3] = fs.nextInt();
        }
        double ans = solve(groups);
        out.format("%.0f\n", ans);
    }

    private static double solve(int[][] groups) {
        long sumx = 0;
        long sumy = 0;
        List<Vector> arr = new ArrayList<>();
        for (int[] v : groups) {
            int x = v[0] - v[1];
            int y = v[2] - v[3];
            if (x == 0 && y == 0) {
        ...
        Collections.sort(arr, (x, y) -> comp(x, y));
        if (test) showItems(arr);

        double ans = squareSumDb(sumx, sumy);
        for (int i = 0; i < m; ++i) {
            Vector v = arr.get(i);
            sumx += v.x;
            sumy += v.y;
            ans = Math.max(ans, squareSumDb(sumx, sumy));
        }
        return ans;
    }

    private static void showItems(List<Vector> arr) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            Vector v = arr.get(i);
            sb.append(String.format("%6d (%10d,%10d)\n", i, v.x, v.y));
        }
        out.print(sb.toString());
    }

    private static double squareSumDb(long sumx, long sumy) {
        double x = Math.abs(sumx);
        double y = Math.abs(sumy);
        double z = x * x + y * y;
        return z;
    }

    private static int comp(Vector a, Vector b) {
        if (a.quad != b.quad) return a.quad - b.quad;
        else return Double.compare(a.ratio, b.ratio);
    }
