...
public class D_Prefix_Permutation_Sums {
    ...
    private static void sagnik() throws IOException {
        ...
        set.add(BigInteger.valueOf(n));
        for (int i = 0; i < n - 1; i++) {
            a[i] = fs.nextBigInteger();
            set.add(BigInteger.valueOf(i + 1));
        }
        int res = 1;
        BigInteger l = BigInteger.valueOf(-1);
        if (set.contains(a[0])) set.remove(a[0]);
        else {
            if (l.equals(BigInteger.valueOf(-1))) l = a[0];
            else res = 0;
        }
        for (int i = 1; i < n - 1; i++) {
            BigInteger subs = a[i].subtract(a[i - 1]);
            if (set.contains(subs)) set.remove(subs);
            else {
                if (l.equals(BigInteger.valueOf(-1))) l = subs;
                else {res = 0; break;}
            }
        }
        if (res == 0) System.out.println("NO");
            ...
            if (l.equals(BigInteger.valueOf(-1))) {
               BigInteger req = BigInteger.valueOf(n).multiply(BigInteger.valueOf(n + 1)).divide(BigInteger.valueOf(2));
                System.out.println(req.subtract(a[n - 2]).equals(sum) ? "YES" : "NO");
            }
            else System.out.println(sum.equals(l) ? "YES" : "NO");
        }
