...
public class B_Imbalanced_Arrays {
    ...
    private static void sagnik() throws IOException {
        ...
        for(int i = 0; i < n; i++) {
            a.add(new Pair(fs.nextInt(), i));
        }
        Collections.shuffle(a);
        Collections.sort(a, (b, c) -> -Integer.compare(b.a, c.a));
        ...
        boolean hasAns = true;
        for(int i = 0; i < n; i++) {
            if(a.get(left).a == n - neg) {
                res[a.get(left).b] = on;
                left++;
                pos++;
            }else if(a.get(right).a - pos == 0) {
                res[a.get(right).b] = -on;
                right--;
                neg++;
            }else {
                hasAns = false;
