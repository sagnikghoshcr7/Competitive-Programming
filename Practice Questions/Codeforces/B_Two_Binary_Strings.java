...
public class B_Two_Binary_Strings {
    ...
    private static void sagnik() throws IOException {
        ...
        if (l.equals(r)) { out.println("YES"); return; }

        int len = l.length();
        boolean ok = false;
        for (int i=len-2; i>=1; i--) {
            if (l.charAt(i) == '0' && r.charAt(i) == '0' && ok == false) { out.println("YES"); return; }
            else if (l.charAt(i) == '1' && r.charAt(i) == '1' && (l.charAt(i - 1) == '0' && r.charAt(i - 1) == '0')) { out.println("YES"); return; }
            else if (l.charAt(i) != r.charAt(i)) ok = true; 
        }
