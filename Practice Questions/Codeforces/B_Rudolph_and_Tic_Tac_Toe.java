...
public class B_Rudolph_and_Tic_Tac_Toe {
    ...
    private static void sagnik() throws IOException {
        ...
        char ans = '.';
        if (a[0][0] == a[0][1] && a[0][1] == a[0][2]) ans = (char) a[0][0];
        if (a[1][0] == a[1][1] && a[1][1] == a[1][2]) ans = (char) a[1][0];
        if (a[2][0] == a[2][1] && a[2][1] == a[2][2]) ans = (char) a[2][0];
        if (a[0][0] == a[1][0] && a[1][0] == a[2][0]) ans = (char) a[0][0];
        if (a[0][1] == a[1][1] && a[1][1] == a[2][1]) ans = (char) a[0][1];
        if (a[0][2] == a[1][2] && a[1][2] == a[2][2]) ans = (char) a[2][2];
        if (a[0][0] == a[1][1] && a[1][1] == a[2][2]) ans = (char) a[0][0];
        if (a[2][0] == a[1][1] && a[1][1] == a[0][2]) ans = (char) a[2][0];
        out.println(ans == '.' ? "DRAW" : ans);
        out.flush();
    }
