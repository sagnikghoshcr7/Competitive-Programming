...
public class C_Kill_the_Monster {
    ...
    private static boolean isDone(long hc, long dc, long hm, long dm){
        double x = Math.ceil(((double)hc)/dm);
        double y = Math.ceil(((double)hm)/dc); 
        return x>=y;
    }
    ...
    private static void sagnik() throws IOException {
        ...
        if(isDone(hc, dc, hm, dm)) out.println("YES");
        else{
            for(int i=0; i<=k; i++){
                if(isDone(hc+(i*a), dc+((k-i)*w), hm, dm)) { flag=0; break; }
            }
