...
public class D_Matrix_Cascade {
    ...
    private static void sagnik() throws IOException {
        int n = fs.nextInt();
        char[][] a = fs.set2sCharArray(n, n);
        
        int res = 0;
        int[][] clone = new int[n + 1][n + 1], vis = new int[n + 1][n + 1], minus = new int[n + 1][n + 1];
        
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (vis[i][j] != 0) { clone[i][j] += vis[i][j]; int max = Math.max(j - 1, 0); vis[i + 1][max] += vis[i][j]; }
                if (minus[i][j] != 0) { clone[i][j] -= minus[i][j]; int min = Math.min(n, j + 1); minus[i + 1][min] += minus[i][j]; }
            }
            for (int j = 1; j < n; j++) clone[i][j] += clone[i][j - 1];
            for (int j = 0; j < n; j++) if (clone[i][j] % 2 == 1) a[i][j] = (a[i][j] == '0' ? '1' : '0'); 
            for (int j = 0; j < n; j++) {
                if (a[i][j] == '1') {
                    res += 1;
                    int max = Math.max(j - 1, 0);
                    int min = Math.min(n, j + 1 + 1);
                    vis[i + 1][max] += 1;
                    minus[i + 1][min] += 1;
                }
            }
