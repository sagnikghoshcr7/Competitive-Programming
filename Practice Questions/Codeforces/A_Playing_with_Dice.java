...
public class A_Playing_with_Dice {
    ...
    private static void sagnik() throws IOException {
        int a = fs.nextInt();
        int b = fs.nextInt();
        int draw = 0, awon = 0, bwon = 0;

        for(int i=1; i<=6; i++){
            if (Math.abs(a-i) == Math.abs(b-i)) draw++;
            else if (Math.abs(a-i) > Math.abs(b-i)) bwon++;
            else awon++;
        }

        out.print(awon + " " + draw + " " + bwon);
        out.flush();
    }
