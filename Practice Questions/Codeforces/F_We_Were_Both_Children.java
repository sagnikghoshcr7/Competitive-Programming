...
public class F_We_Were_Both_Children {
    ...
    private static void sagnik() throws IOException {
        ...
        Map<Integer, Integer> map = new HashMap<>(), res = new HashMap<>();

        for (int i = 0; i < n; i++) map.put(a[i], map.getOrDefault(a[i], 0) + 1);
        for (Map.Entry<Integer, Integer> entry : map.entrySet()) {
            for (int i=entry.getKey(); i <= n; i+=entry.getKey()) res.put(i, res.getOrDefault(i, 0) + entry.getValue());
        }

        int ans = 0;
        for (int it : res.values()) ans = Math.max(ans, it);
        out.println(ans);
    }
