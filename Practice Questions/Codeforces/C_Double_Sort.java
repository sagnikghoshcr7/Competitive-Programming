...
public class C_Double_Sort {
    ...
    private static void sagnik() throws IOException {
        ...
        for(int i=0;i<n-1;i++){
            ...
            int smallest_a_value = a[i];
            for(int j=i+1;j<n;j++){
                if(smallest_a_value>a[j]){
                    b_value = b[j];
                    smallest_index = j;
                    smallest_a_value = a[j];
                }
                else if(smallest_a_value == a[j] && b_value>b[j]){
                    b_value = b[j];
                    smallest_index = j;
                }
            }
            swap(a,i,smallest_index);
            swap(b,i,smallest_index);
            if(i!=smallest_index) list.add(List.of(i+1, smallest_index+1));
        }
        if(!isArraySorted(b)) out.println(-1);
        else {
            out.println(list.size());
            for(int i=0;i<list.size();i++){
                out.println(list.get(i).get(0)+ " "+ list.get(i).get(1));
            }
        }
    }
