...
public class B_Array_merging {
    ...
    private static void sagnik() throws IOException {
        ...
        for (int i = 1; i < n; ++i) {
            if (a[i - 1] == a[i]) {
                cnt++;
                m1.put(a[i], Math.max(cnt, m1.getOrDefault(a[i], 0)));
            } else {
                cnt = 1;
                m1.put(a[i], Math.max(cnt, m1.getOrDefault(a[i], 0)));
            }
        ...
        for (int i = 1; i < n; ++i) {
            if (b[i - 1] == b[i]) {
                cnt++;
                m2.put(b[i], Math.max(cnt, m2.getOrDefault(b[i], 0)));
            } else {
                cnt = 1;
                m2.put(b[i], Math.max(cnt, m2.getOrDefault(b[i], 0)));
            }
        ...
        for (int key : m1.keySet()) {
            if (m2.containsKey(key)) {
                int sum = m1.get(key) + m2.get(key);
                if (sum > maxSum) maxSum = sum;
            }
            else maxSum = Math.max(maxSum,m1.get(key));
        }
