    ...
    private static void sagnik() throws IOException {
        ...
        while (t-- > 0) {
            ...
            if (type == 1) {
                ...
                if (sum == target) { out.println("YES"); continue; }
                if (sum < target) { out.println("NO"); continue; }

                int lastKey = Integer.MAX_VALUE;
                while (target > 0 && map.floorEntry(Math.min(target, lastKey)) != null) {
                    int key = map.floorKey(Math.min(target, lastKey));
                    int count = map.get(key);
                    long canDecrease = (long) key * count;
                    if (canDecrease > target) canDecrease = (target / key) * key;
                    target -= canDecrease;
                    lastKey = key - 1;
                }
