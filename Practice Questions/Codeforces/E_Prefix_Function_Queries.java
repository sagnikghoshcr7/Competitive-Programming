...
public class E_Prefix_Function_Queries {
    ...
    private static void sagnik() throws IOException {
        String st = fs.next();
        int n = st.length();
        byte[] s = new byte[n + 10];
        for (int i = 0; i < n; i++) {
            s[i] = (byte) (st.charAt(i) - 'a');
        }
        int[] pi = new int[n + 10];
        int[][] cache = new int[n + 10][26];
        for (int[] i : cache) {
            Arrays.fill(i, -1);
        }
        prefixFunction(cache, s, pi, n, 1);
        s = Arrays.copyOf(s, n + 10);
        int q = fs.nextInt();
        for (; q > 0; q--) {
            String ss = fs.next();
            int nn = ss.length();
            for (int i = 0; i < nn; i++) {
                s[i + n] = (byte) (ss.charAt(i) - 'a');
                Arrays.fill(cache[i + n], -1);
            }
            prefixFunction(cache, s, pi, n + nn, n);
            for (int i = 0; i < nn; i++) {
                out.print(pi[i + n] + " ");
            }
            out.println();
    ...
    private static void prefixFunction(int[][] cache, byte[] s, int[] pi, int n, int from) {
