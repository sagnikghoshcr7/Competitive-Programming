...
public class A_Vasya_and_Book {
    ...
    private static void sagnik() throws IOException {
        ...
        int n = fs.nextInt(), x = fs.nextInt(), y = fs.nextInt(), d = fs.nextInt();
        int l = Math.abs(x - y);
        if (l % d == 0) count = Math.min(count, distance(x, y, d));
        l = y - 1;
        if (l % d == 0) count = Math.min(count, distance(x, 1, d) + distance(1, y, d));
        l = n - y;
        if (l % d == 0) count = Math.min(count, distance(x, n, d) + distance(n, y, d));

        out.println(count == Integer.MAX_VALUE ? -1 : count);
    }

    private static int distance(int x, int y, int d) {
        return (Math.abs(x - y) + Math.abs(d - 1)) / d;
    }
