...
public class C_Particles {
    ...
    private static void sagnik() throws IOException {
        ...
        // if all values are negetive then print max one else print max (sum of positive even indexed elements, sum of positive odd indexed elements)
        long oddSum = 0, evenSum = 0, negetiveCount = 0, maxOfMin = Integer.MIN_VALUE;
        for (int i=0; i<n; i++) {
            if (a[i] >= 0) {
                if ((i&1) == 1) oddSum += a[i];
                else evenSum += a[i];
            }
            else { negetiveCount++; maxOfMin = Math.max(maxOfMin, a[i]); }
        }

        if (negetiveCount == n) { out.println(maxOfMin); return; }
        out.println(Math.max(oddSum, evenSum));
    }
