...
public class B_Same_Parity_Summands {
    ...
    private static void sagnik() throws IOException {
        ...
        if (!(check_even(n, k) || check_odd(n, k))) out.println("NO");
    }

    private static boolean check_even(int n, int k) {
        int rem = n - 2 * k;

        if (rem < 0) return false;
        if ((rem & 1) == 1) return false;

        out.println("YES");

        for (int i = 0; i < k - 1; ++i) out.print(2 + " ");
        out.println();
        out.println(2 + rem);
    ...
    private static boolean check_odd(int n, int k) {
        int rem = n - k;
        ...
        for (int i = 0; i < k - 1; ++i) out.print(1 + " ");
        out.println();
        out.println(1 + rem);
