...
public class D_Sorting_By_Multiplication {
    ...
    private static void sagnik() throws IOException {
        ...
        out.println(Math.min(helper(0, n, dp, a, 0), helper(0, n, dp, a, 1)));
    }

    private static int helper(int i, int n, int[][] dp, int[] a, int ind) {
        if (i == n - 1) { if (ind == 1) return 0; return 1; }
        if (dp[i][ind] != -1) return dp[i][ind];
        if (ind == 0) {
            int res = Integer.MAX_VALUE;
            if (a[i + 1] < a[i]) res = helper(i + 1, n, dp, a, 0);
            else res = helper(i + 1, n, dp, a, 0) + 1;
            res = Math.min(helper(i + 1, n, dp, a, 1) + 1, res);
            return dp[i][ind] = res;
        } else {
            int res = Integer.MAX_VALUE;
            if (a[i + 1] > a[i]) res = helper(i + 1, n, dp, a, 1);
            else res = helper(i + 1, n, dp, a, 1) + 1;
            return dp[i][ind] = res;
        }
    }
