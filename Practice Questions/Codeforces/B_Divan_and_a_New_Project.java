...
public class B_Divan_and_a_New_Project {
    ...
    private static void sagnik() throws IOException {
        ...
        visits.add(new Visit(0, 0)); // Placeholder to match 1-based index
 
        for (int i = 1; i <= n; i++) {
            int times = fs.nextInt();
            visits.add(new Visit(times, i));
        }
 
        // Sort visits by times in descending order
        Collections.sort(visits.subList(1, visits.size()));
 
        long totalTime = 0;
        long curCoord = 1;
        long[] config = new long[n + 1];
 
        for (int i = 1; i <= n; i++) {
            totalTime += 2 * curCoord * visits.get(i).times;
 
            if (i % 2 == 1) { // Odd i
                config[visits.get(i).id] = curCoord;
            } else { // Even i
                config[visits.get(i).id] = -curCoord;
                curCoord++;
            }
    ...
    public static class Visit implements Comparable<Visit> {
        int times; // #times to visit
        int id;    // id (1...n)
 
        Visit(int times, int id) {
            this.times = times;
            this.id = id;
        }
        ...
        public int compareTo(Visit other) {
            return Integer.compare(other.times, this.times); // Sort in descending order of 'times'
        }
    }
