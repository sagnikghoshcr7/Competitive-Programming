...
public class C_Divisor_Chain {
    ...
    private static void sagnik() throws IOException {
        ...
        list.add(n);
        long lo = 0, hi = 50, ans = -1;
        while (lo <= hi) {
            long mid = (lo + hi) / 2;
            long ok = 1;
            for (int i=0; i<mid; i++) ok *= 2;

            if (ok <= n) { ans = mid; lo = mid + 1; }
            else hi = mid - 1;
        }

        long num = 1;
        for (int i=0; i<ans; i++) num *= 2;
        long stor = n;
        n -= num;
        for (int i = 0; i < 31; i++) {
            if (((1L << i) & n) != 0) { list.add(stor - (1L << i)); stor -= (1L << i); }
        }
        while (num != 1) {
            list.add(num / 2);
            num /= 2;
    ...
    private static void prtList(List<Long> x) {for(long i : x) {System.out.print(i+" ");}}
    private static void prtArr(long[] A) {for(int i=0;i<A.length;i++)System.out.print(A[i]+" ");}
