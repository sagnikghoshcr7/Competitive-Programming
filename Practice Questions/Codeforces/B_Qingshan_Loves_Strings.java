...
public class B_Qingshan_Loves_Strings {
    ...
    private static void sagnik() throws IOException {
        ...
        boolean oneResolve = false;
        boolean zeroResolve = false;

        if (b.charAt(0) == '1' && b.charAt(m-1) == '1') zeroResolve = true;
        if (b.charAt(0) == '0' && b.charAt(m-1) == '0') oneResolve = true;
        ...
        for (int i=0; i<n-1; i++) {
            if (a.charAt(i) == '1' && a.charAt(i+1) == '1') {
                if (!oneResolve) { out.println("No"); return; }
            }

            if (a.charAt(i) == '0' && a.charAt(i+1) == '0') {
                if (!zeroResolve) { out.println("No"); return; }
            }
        }
