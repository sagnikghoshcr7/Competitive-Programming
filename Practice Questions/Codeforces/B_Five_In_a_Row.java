...
public class B_Five_In_a_Row {
    ...
    private static void sagnik() throws IOException {
		...
		for (int i = 0; i < 10; i++)
			for (int j = 2; j < 8; j++)
				if (check(cc[i][j - 2], cc[i][j - 1], cc[i][j], cc[i][j + 1], cc[i][j + 2])) { out.println("YES"); return; }
		for (int i = 2; i < 8; i++)
			for (int j = 0; j < 10; j++)
				if (check(cc[i - 2][j], cc[i - 1][j], cc[i][j], cc[i + 1][j], cc[i + 2][j])) { out.println("YES"); return; }
		for (int i = 2; i < 8; i++)
			for (int j = 2; j < 8; j++)
				if (check(cc[i - 2][j - 2], cc[i - 1][j - 1], cc[i][j], cc[i + 1][j + 1], cc[i + 2][j + 2]) || check(cc[i - 2][j + 2], cc[i - 1][j + 1], cc[i][j], cc[i + 1][j - 1], cc[i + 2][j - 2])) { out.println("YES"); return; }
		out.println("NO");
    }

    
	private static boolean check(byte a, byte b, byte c, byte d, byte e) {
		if (a == 'O' || b == 'O' || c == 'O' || d == 'O' || e == 'O') return false;
		int k = 0;
		if (a == '.') k++;
		if (b == '.') k++;
		if (c == '.') k++;
