...
public class G_The_Morning_Star {
    ...
    private static void sagnik() throws IOException {
        int n = fs.nextInt();
        Map<Integer, Integer> x1 = new HashMap<>(), y1 = new HashMap<>(), x2 = new HashMap<>(), y2 = new HashMap<>();

        for (int i = 0; i < n; i++) {
            int a = fs.nextInt(), b = fs.nextInt();
            x1.put(a, x1.getOrDefault(a, 0) + 1);
            y1.put(b, y1.getOrDefault(b, 0) + 1);
            x2.put(a + b, x2.getOrDefault(a + b, 0) + 1);
            y2.put(b - a, y2.getOrDefault(b - a, 0) + 1);
        }

        long count = 0;
        count += countPairs(x1)+countPairs(y1)+countPairs(x2)+countPairs(y2);
        out.println(count);
    }

    private static long countPairs(Map<Integer, Integer> map) {
        long count = 0;
        for (int q : map.values()) count += (long) q * (q - 1);
        return count;
    }
