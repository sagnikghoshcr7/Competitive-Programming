...
public class Ultimate_Fencing {
    ...
    private static void sagnik() throws IOException {
        ...
        SortedSet<Integer> map = new TreeSet<>(), map2 = new TreeSet<>();
        map.add(0); map.add(m);
        map2.add(m);
        ...
        for (int i = 0; i < n; i++) {
            int x = map.headSet(a[i]).last();
            int y = map.tailSet(a[i]).first();
            freq.put(y - x, freq.getOrDefault(y - x, 0) - 1);
            if (freq.get(y - x) == 0) map2.remove(y - x);

            map2.add(a[i] - x); map2.add(y - a[i]);
            freq.put(a[i] - x, freq.getOrDefault(a[i] - x, 0) + 1);
            freq.put(y - a[i], freq.getOrDefault(y - a[i], 0) + 1);
            map.add(a[i]);
            out.print(map2.last() + " ");
        }
        out.println();
