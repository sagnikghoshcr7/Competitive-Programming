...
public class SpiralMatrix020723 {
    ...
    private static void sagnik() throws IOException {
        ...
        while (count > 0) {
            // printing 1st row
            for (int i=fcol; i<=lcol && count > 0; i++) {
                out.print(arr[frow][i] + " ");
                count--;
            }
            frow++;
    
            // printing last col
            for (int i=frow; i<=lrow && count > 0; i++) {
                out.print(arr[i][lcol] + " ");
                count--;
            }
            lcol--;

            // printing last row
            for (int i=lcol; i>=fcol && count > 0; i--) {
                out.print(arr[lrow][i] + " ");
                count--;
            }
            lrow--;

            // printing 1st col
            for (int i=lrow; i>= frow && count > 0; i--) {
                out.print(arr[i][fcol] + " ");
                count--;
            }
            fcol++;
        }
