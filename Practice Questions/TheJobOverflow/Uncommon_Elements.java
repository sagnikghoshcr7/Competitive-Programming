...
public class Uncommon_Elements {
    ...
    private static void sagnik() throws IOException {
        ...
        for (int x : b) map2.put(x, map2.getOrDefault(x, 0) + 1);

        Set<Integer> temp = new HashSet<>();
        for (int x : a) temp.add(x);
        for (int x : b) temp.add(x);

        for (int x : temp) {
            if (map1.containsKey(x) && !map2.containsKey(x)) ans.add(x);
            if (map2.containsKey(x) && !map1.containsKey(x)) ans.add(x);
        }
