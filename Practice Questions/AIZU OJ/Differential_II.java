...
public class Differential_II {
    ...
    private static void swap(char[] arr, int i, int j) { int temp = arr[i]; arr[i] = arr[j]; arr[j] = (char) temp; }
    private static void reverse(char[] arr) {int s=0,e=arr.length-1;while(s<e){swap(arr,s,e);s++;e--;}}
