...
public class aliennumbers {
    ...
    private static void sagnik() throws IOException {
        ...
        out.println("Case #" + (iterator++) + ": " + solve(num, from, to));
    }

    private static int convTo10(String num, String base) {
        int baseLength = base.length();
        int[] fromBase = new int[200];
        for (int i=0; i<baseLength; i++) {
            fromBase[base.charAt(i)] = i;
        }
        int res = 0;
    ...
    private static String solve(String num, String from, String to) {
        char[] toBase = new char[200];
        int toBaseLength = to.length();
        to.getChars(0, toBaseLength, toBase, 0);
        int b10 = convTo10(num, from);
        StringBuilder res = new StringBuilder();
        while (b10 > 0) {
            res.append(toBase[b10 % toBaseLength]);
            b10 /= toBaseLength;
        }
