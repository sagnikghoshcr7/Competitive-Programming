...
public class C_XX_to_XXX {
    ...
    private static void sagnik() throws IOException {
        ...
        if (t.length < s.length) {out.print("No"); return;}

        int count = 0;
        for (char c : t) {
            if (count < s.length && c == s[count]) {count++; continue;}
            if (count < 2) {out.print("No"); return;}
            if (c == s[count - 1] && s[count - 1] == s[count - 2]) continue;
            else {out.print("No"); return;}
        }
        out.print("Yes");
    }
