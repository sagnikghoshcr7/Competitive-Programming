...
public class D_Knapsack_1 {
    ...
    private static void sagnik() throws IOException {
        ...
        for(int i=0; i<n; i++) {
            weight[i] = fs.nextInt(); value[i] = fs.nextInt();
        }

        long[][] dp = new long[n+1][w+1]; // dp[i][j] -> most points can get with (0 to i) items and (<=j) weight


        for (int i=1; i<=n; i++) {
            for (int sum_w=1; sum_w<=w; sum_w++) {
                if (sum_w < weight[i - 1]) { // If we don't include the i-th item
                    dp[i][sum_w] = dp[i - 1][sum_w];
                } else { // If we include the i-th item
                    dp[i][sum_w] = Math.max(dp[i - 1][sum_w], dp[i - 1][sum_w - weight[i - 1]] + value[i - 1]);
                }
            }
        }
