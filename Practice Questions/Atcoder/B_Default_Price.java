...
public class B_Default_Price {
    ...
    private static void sagnik() throws IOException {
        int n = fs.nextInt();
        int m = fs.nextInt();
		String[] c = new String[n]; String[] d = new String[m]; int[] p = new int[m];
		for (int i = 0; i < n; i++) c[i] = fs.next();		
		for (int i = 0; i < m; i++) d[i] = fs.next();
		int p0 = fs.nextInt();		
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				if (c[i].equals(d[j])) { ans += p[j]; continue label; }
			}
			ans += p0;
		}
		out.print(ans);
