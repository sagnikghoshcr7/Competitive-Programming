...
public class C_RANDOM {
    ...
    private static void sagnik() throws IOException {
        ...
        String[] TransportS = new String[W];
        String[] TransportT = new String[W];

        for (int i = 0; i < W; i++) {
            String[] tmpS = new String[H];
            String[] tmpT = new String[H];
            for (int j = 0; j < H; j++) {
                tmpS[j] = S[j][i];
                tmpT[j] = T[j][i];
            }
            TransportS[i] = String.join("", tmpS);
            TransportT[i] = String.join("", tmpT);
        }

        Arrays.sort(TransportS);
        Arrays.sort(TransportT);

        System.out.println(Arrays.deepEquals(TransportS, TransportT) ? "Yes" : "No");
    }
