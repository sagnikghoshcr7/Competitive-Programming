...
public class B_ABC_DEF {
    ...
    private static void sagnik() throws IOException {
        BigInteger n = new BigInteger("998244353");
        Scanner sc = new Scanner(System.in);
        BigInteger[] a = new BigInteger[6];
        for (int i = 0; i < 6; i++) a[i] = fs.nextBigInteger();

        BigInteger m = ((a[0].multiply(a[1]).multiply(a[2])).subtract(a[3].multiply(a[4]).multiply(a[5]))).remainder(n);
        out.println(m);
        out.flush();
    }
