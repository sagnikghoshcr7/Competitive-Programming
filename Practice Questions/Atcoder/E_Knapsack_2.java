...
public class E_Knapsack_2 {
    ...
    private static void sagnik() throws IOException {
        ...
        long[][] dp = new long[n + 1][100001]; // dp[i][j] -> most points can get with (0 to i) items and (<=j) weight
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= 100000; j++) {
                int wc = value[i - 1]; // weight of the i-th item
                if (j < wc) dp[i][j] = dp[i - 1][j];
                else dp[i][j] = Math.min(dp[i - 1][j], weight[i - 1] + dp[i - 1][j - wc]);
                if (dp[i][j] <= w) ans = j;
            }
        }
