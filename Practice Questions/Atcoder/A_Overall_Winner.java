...
public class A_Overall_Winner {
    ...
    private static void sagnik() {
        ...
        boolean mx = false;
        for (int i=0; i<n; i++) {
            if (s.charAt(i) == 'T') {cntT++; if (cntT > cntA) mx = true;}
            else {cntA++; if (cntT > cntA) mx = false;};
        }
        if (cntA > cntT) System.out.println("A");
        else if (cntA < cntT) System.out.println("T");
        else {
            System.out.println(mx ? "T" : "A");
        }
    }
}