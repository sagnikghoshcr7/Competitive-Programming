...
public class C_Make_Takahashi_Happy {
    ...
    private static void sagnik() throws IOException {
        ...
        w = fs.nextInt();
        a = new int[h+1][w+1];
        for (int i=1; i<=h; i++) for (int j=1; j<=w; j++) a[i][j] = fs.nextInt();

        helper(1, 1, new HashSet<Integer>());
        out.print(ans);        
    }

    private static void helper(int x, int y, Set<Integer> num) {
        if (x <= h || y <= w) {
            if (!num.contains(a[x][y])) {
                num.add(a[x][y]);
                if (x<h) helper(x+1, y, new HashSet<Integer>(num));
                if (y<w) helper(x, y+1, new HashSet<Integer>(num));
                if (x == h && y == w) ans++;
            }
        }
    }
