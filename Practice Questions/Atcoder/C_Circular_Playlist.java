...
public class C_Circular_Playlist {
    ...
    private static long arrSum(long[] A) {return Arrays.stream(A).sum();}
    ...
    private static void sagnik() throws IOException {
        ...
        long[] A = fs.lsetArray((int)n);

        time %= arrSum(A);
        long now = 0;
        for (int i=0; i<n; i++) {
            now += A[i];
            if (now > time) {System.out.println((i+1) + " " + (A[i] - (now-time))); return;}
        }
    }
}