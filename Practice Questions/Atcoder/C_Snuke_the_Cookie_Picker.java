...
public class C_Snuke_the_Cookie_Picker {
    ...
    private static void sagnik() throws IOException {
		...
		for(int i=1; i<=h; i++) {
			for(int j=1; j<=w; j++) {
				int coockieCnt = 0;
				if(grid[i][j] == '.') {
					if (0 < i-1 && grid[i-1][j] == '#') coockieCnt++;
					if (i+1 <= h && grid[i+1][j] == '#') coockieCnt++;
					if (j+1 <= w && grid[i][j+1] == '#') coockieCnt++;
					if (0 < j-1 && grid[i][j-1] == '#') coockieCnt++;
				}
