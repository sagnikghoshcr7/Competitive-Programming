...
public class B_Hammer {
    ...
    private static void sagnik() throws IOException {
        int destination = fs.nextInt();
        int wall = fs.nextInt();
        int hammer = fs.nextInt();

        if(wall < 0) {destination *= -1; wall *= -1; hammer *= -1;}

        if(destination < wall) out.print(Math.abs(destination));
        else {
            if(wall < hammer) out.print(-1);
            else out.print(Math.abs(hammer) + Math.abs(destination - hammer));
        }
