...
public class C_Remembering_the_Days {
    ...
    private static void sagnik() throws IOException {
        ...
        for (int i=0; i<m; i++) {
            ...
            graph[u][v] = graph[v][u] = fs.nextInt();
        }
        ...
        for (int i=0; i<n; i++) {
            ...
            max = Math.max(max, findMaxWeight(i, 0));
        }

        out.print(max);        
    }

    private static int findMaxWeight(int node, int weight) {
        vis[node] = true;
        int max = weight;

        for (int i=0; i<graph.length; i++) {
            if (!vis[i] && graph[node][i] > 0) {
                max = Math.max(max, findMaxWeight(i, weight + graph[node][i]));
            }
        }
        vis[node] = false;
        return max;
