...
public class C_Flavors {
    ...
    private static void sagnik() throws IOException {
        int n = fs.nextInt();
        int[][] arr = fs.set2dArray(n, 2);

        Arrays.sort(arr, (a, b) -> {
            return b[1] - a[1];
        });

        long total = arr[0][1], prev = arr[0][0], ans = arr[0][1];

        for (int i=1; i<n; i++) {
            if (arr[i][0] != prev) { total += arr[i][1]; break; }
            else ans = Math.max(ans, total + arr[i][1]/2);
        }

        out.print(Math.max(ans, total));
    }
