...
public class C_Cards_Query_Problem {
    ...
    private static void sagnik() throws IOException {
        ...
        List<List<Integer>> box = new ArrayList<>(n+10);
        List<TreeSet<Integer>> card = new ArrayList<>(200100);

        for (int i=0; i<n+10; i++) box.add(new ArrayList<>());
        for (int i=0; i<200100; i++) card.add(new TreeSet<>());

        for (int i=0; i<q; i++) {
            int query = fs.nextInt();
            if (query == 1) {
                int cardNum = fs.nextInt();
                int boxNum = fs.nextInt();

                box.get(boxNum).add(cardNum);
                card.get(cardNum).add(boxNum);
            }
            else if (query == 2) {
                int boxNum = fs.nextInt();
                Collections.sort(box.get(boxNum));

                for (int k=0; k<box.get(boxNum).size(); k++) {
                    out.print(box.get(boxNum).get(k) + " ");
                }
                out.println();
            }
            else {
                int cardNum = fs.nextInt();
                List<Integer> sortedList = new ArrayList<>(card.get(cardNum));

                for (int k=0; k<sortedList.size(); k++) {
                    out.print(sortedList.get(k) + " ");
                }
                out.println();
            }
        }        
