...
public class B_Tournament_Result {
    ...
    private static void sagnik() throws IOException {
		...
		for(int i=0; i<n; i++) {
		    for(int j=0; j<n; j++) {
		        if(s[i][j]=='W' && s[j][i]!='L') {System.out.print("incorrect"); return;}
		        if(s[i][j]=='L' && s[j][i]!='W') {System.out.print("incorrect"); return;}
		        if(s[i][j]=='D' && s[j][i]!='D') {System.out.print("incorrect"); return;}
		        if(s[i][j]=='-' && s[j][i]!='-') {System.out.print("incorrect"); return;}
		    }
		}
