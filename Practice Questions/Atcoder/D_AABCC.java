...
public class D_AABCC {
    static int MAXN = 288676;
    static long[] primes = new long[MAXN];
            ...
            for (int j = i + 1; j <= cnt; ++j) {
                for (int k = j + 1; k <= cnt; ++k) {
                    if (primes[i] * primes[i] * primes[j] > n ||
                            primes[i] * primes[i] * primes[j] * primes[k] > n ||
                            primes[i] * primes[i] * primes[j] * primes[k] * primes[k] > n) {
                        break;
                    }
                    ans++;
                }
