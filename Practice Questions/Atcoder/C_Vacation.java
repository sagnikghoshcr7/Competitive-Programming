...
public class C_Vacation {
    ...
    private static void sagnik() throws IOException {
        int n = fs.nextInt();
        int[][] a = fs.set2dArray(n, 3);

        int[][] dp = new int[n][3]; // dp[i][j] -> from 0 to i days & from 0 to j task max points with no consecutive task
        ...
        for (int i=1; i<n; i++) {
            for (int j=0; j<3; j++) {
                ...
                for (int task=0; task<3; task++) {
                    if (j != task) max = Math.max(max, dp[i-1][task]);
                }
                dp[i][j] = a[i][j] + max;
            }
        }
