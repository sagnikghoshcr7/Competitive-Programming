...
public class B_Vacation_Together {
    ...
    private static void sagnik() throws IOException {
        ...
        for (int i=0; i<n; i++) {
            String s = fs.next();
            for (int j=0; j<d; j++) a[i][j] = s.charAt(j);
        }

        int consicutiveDays = 0;
        int days = 0;
        for (int i=0; i<d; i++) {
            int count = 0;
            for (int j=0; j<n; j++) {
                if (a[j][i] == 'x') {days = 0; break;}
                else count++;
            }
            if (count == n) days++;
            consicutiveDays = Math.max(consicutiveDays, days);
        }
        out.print(consicutiveDays);
    }
